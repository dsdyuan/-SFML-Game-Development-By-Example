...
Character::Character(EntityManager * l_entityMgr)
	: EntityBase(l_entityMgr), m_spriteSheet(m_entityManager->GetContext()->m_textureManager), m_jumpVelocity(250), m_hitPoints(5)
{
	m_name = "Character";
}
...
void Character::Load(const std::string & l_path)
{
	...
	file.open(std::string("Characters/") + l_path);
...
{
	// changed the ternary operator for readability
	if (m_spriteSheet.GetDirection() == Direction::Left)
		m_attackAABB.left = (m_AABB.left - m_attackAABB.width) - m_attackAABBoffset.x;
	else
		m_attackAABB.left = (m_AABB.left + m_attackAABB.width) + m_attackAABBoffset.x;
