...
Map::Map(SharedContext * l_context, BaseState * l_currentState)
	: m_context(l_context), m_defaultTile(l_context), m_maxMapSize(32, 32), m_tileCount(0), m_tileSetCount(0), m_mapGravity(512.0f), m_loadNextMap(false), m_currentState(l_currentState)
{
...
void Map::LoadMap(const std::string & l_path)
{
	...
	std::cout << "Loading map " << l_path << std::endl;
	...
	while (std::getline(mapFile, line))
	{
		...
		if (type == "TILE")
		{
			...
			{
				std::cout << "! Duplicate tile at " << tileCoords.x
...
void Map::Draw()
{
	...
	sf::Vector2i tileBegin(static_cast<int>(floor(viewSpace.left / Sheet::TileSize)),
		static_cast<int>(floor(viewSpace.top / Sheet::TileSize)));
	sf::Vector2i tileEnd(static_cast<int>(ceil((viewSpace.left + viewSpace.width) / Sheet::TileSize)),
		static_cast<int>(ceil((viewSpace.top + viewSpace.height) / Sheet::TileSize)));
	...
	for (int x = tileBegin.x; x <= tileEnd.x; ++x)
	{
		for (int y = tileBegin.y; y <= tileEnd.y; ++y)
		{
			...
			sprite.setPosition(x * static_cast<float>(Sheet::TileSize),
				y * static_cast<float>(Sheet::TileSize));
...
void Map::LoadTiles(const std::string & l_path)
{
	...
	if (!file.is_open())
	{
		std::cout << "! Failed loading tileset file: " << l_path << std::endl;
		return;
	}
