...
Player::~Player()
{
	...
	events->RemoveCallback(StateType::Game, "Player_MoveJump");
	events->RemoveCallback(StateType::Game, "Player_MoveAttack");
}
...
void Player::OnEntityCollision(EntityBase * l_collider, bool l_attack)
{
	...
	if (l_attack)
	{
		...
		Character* opponent = static_cast<Character*>(l_collider);
		opponent->GetHurt(1);
